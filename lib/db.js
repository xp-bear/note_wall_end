...
// 查询留言墙或照片墙的总条数。
exports.findWallPhotoTotal = (type, label) => {
  let _sql;
  if (label == -1) {
    _sql = `select count(*) as totalNumber from walls where type=${type};`;
  } else {
    _sql = `select count(*) as totalNumber from walls where type=${type} and label=${label};`;
  }
  return query(_sql);
};
//查询倒叙分页查墙的评论
...
// 删除相关图片或者视频资源。
exports.deletePhoto = (urlpath) => {
  return new Promise((resolve, reject) => {
    let filename = urlpath.split("/photo")[1]; // 取出/file-1672218723917.mp4
    let pathname = __dirname.replace("lib", "data/photo");
